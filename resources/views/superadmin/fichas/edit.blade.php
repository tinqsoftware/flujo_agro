...
@section('page-subtitle', 'Modifica los datos de la ficha en el sistema')
...
<div class="row">
    <div class="col-lg-8">
        <form method="POST" action="{{ route('fichas.update', $ficha) }}" id="fichaForm">
            @csrf
            @method('PUT')
            ...
            <div class="card mb-4">
                ...
                <div class="card-body">
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            ...
                            <input type="text" class="form-control @error('nombre') is-invalid @enderror" 
                                   id="nombre" name="nombre" value="{{ old('nombre', $ficha->nombre) }}" 
                            ...
                            <select class="form-select @error('id_emp') is-invalid @enderror" 
                                ...
                                @foreach($empresas as $empresa)
                                    <option value="{{ $empresa->id }}" 
                                        {{ (old('id_emp', $ficha->id_emp) == $empresa->id) ? 'selected' : '' }}>
                                        {{ $empresa->nombre }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        @else
                        <input type="hidden" name="id_emp" value="{{ $ficha->id_emp }}">
                        @endif
                    </div>
                    ...
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            ...
                            <div class="form-control-plaintext bg-light rounded p-2 border">
                                ...
                                <small class="text-muted ms-2">No se puede modificar el tipo de una ficha existente</small>
                            </div>
                            <!-- Campo oculto para enviar el valor -->
                            <input type="hidden" name="tipo" value="{{ $ficha->tipo }}">
                        </div>
                        
                        <div class="col-md-6 mb-3" id="flujoContainer" 
                            style="display: {{ in_array($ficha->tipo, ['Flujo', 'Etapa']) ? 'block' : 'none' }};">
                    ...
                    <div class="row" id="etapaContainer" 
                        style="display: {{ $ficha->tipo === 'Etapa' ? 'block' : 'none' }};">
                        <div class="col-md-6 mb-3">
            ...
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    <div id="noCamposMessage" class="text-center py-4" style="display: none;">
...
<!-- Datos existentes de la ficha para JavaScript -->
<script>
    window.fichaData = {
        atributos: @json($atributos),
        contexto: @json($contexto),
        tipo: '{{ $ficha->tipo }}',
        empresaId: {{ $ficha->id_emp }}
    };
</script>
@endsection
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function setupEventListeners() {
        ...
        // No necesitamos listener para tipo ya que no es editable
    ...
    function loadExistingData() {
        const fichaData = window.fichaData;
        
        // Cargar flujos si es necesario
        if (['Flujo', 'Etapa'].includes(fichaData.tipo)) {
            loadFlujosByEmpresa(fichaData.empresaId, function() {
                if (fichaData.contexto && fichaData.contexto.id_flujo) {
                    document.getElementById('id_flujo').value = fichaData.contexto.id_flujo;
                    
                    if (fichaData.tipo === 'Etapa') {
                        loadEtapasByFlujo(fichaData.contexto.id_flujo, function() {
                            if (fichaData.contexto.id_etapa) {
                                document.getElementById('id_etapa').value = fichaData.contexto.id_etapa;
                            }
                        });
                    }
                }
        ...
        fichaData.atributos.forEach(function(atributo) {
            addCampoWithData(atributo);
        });
        
        // Si no hay campos, mostrar mensaje
        if (fichaData.atributos.length === 0) {
            noCamposMessage.style.display = 'block';
        }
    }
    
    function addCampoWithData(atributo) {
        campoCounter++;
        ...
        campoItem.querySelector('.campo-number').textContent = atributo.nro;
        
        // Llenar datos existentes
        campoItem.querySelector('.campo-nombre').value = atributo.titulo;
        campoItem.querySelector('.campo-tipo').value = atributo.tipo;
        campoItem.querySelector('.campo-nro').value = atributo.nro;
        
        // Configurar ancho si aplica
        if (atributo.ancho) {
            const anchoInput = campoItem.querySelector('.campo-ancho');
            if (anchoInput) {
                anchoInput.value = atributo.ancho;
            }
        }
        
        // Configurar obligatorio
        if (atributo.obligatorio) {
            campoItem.querySelector('input[type="checkbox"]').checked = true;
        }
        
        // Configurar opciones si aplica
        if (atributo.json && ['radio', 'desplegable', 'checkbox'].includes(atributo.tipo)) {
            const opciones = Array.isArray(atributo.json) ? atributo.json : JSON.parse(atributo.json);
            loadOpcionesInCampo(campoItem, opciones);
        }
        ...
        // Mostrar configuraciones apropiadas
        handleCampoTipoChange(campoItem, atributo.tipo);
    ...
    function loadOpcionesInCampo(campoItem, opciones) {
        ...
        opciones.forEach(function(opcion, index) {
            ...
            opcionDiv.innerHTML = `
                <input type="text" class="form-control opcion-input" placeholder="Opción ${index + 1}" value="${opcion}">
                <button type="button" class="btn btn-outline-danger remove-opcion">
                    <i class="fas fa-times"></i>
                </button>
            `;
    ...
    function handleEmpresaChange() {
        ...
        if (empresaId) {
            // Cargar flujos de la empresa si el tipo actual lo requiere
            const tipoActual = '{{ $ficha->tipo }}';
            if (['Flujo', 'Etapa'].includes(tipoActual)) {
    ...
    function handleTipoChange() {
        ...
        const empresaId = document.getElementById('id_emp')?.value || '{{ $ficha->id_emp }}';
    ...
    function loadFlujosByEmpresa(empresaId, callback) {
    ...
    function loadEtapasByFlujo(flujoId, callback) {
    ...
    function updateCampoNumbers() {
        ...
        campos.forEach((campo, index) => {
            ...
            campo.querySelector('.campo-nro').value = number;
